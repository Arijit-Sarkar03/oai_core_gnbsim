...
```
`$USER`should be replaced with your VM username, to know your username run the command`whoami` in your terminal.
...
- The component repo has all the docker images required for this setup
- `cd ../docker-compose`
...
## Single Host setup
This setup deployment is done in a single host VM which means the `oai-core` and the `gnbsim` is in the same VM

Follow the steps mentioned below:

### Execute with docker compose
Edit required changes in environment variables of `docker-compose-basic-nrf.yaml` file for experimentation.
...
```
- Note: Compare all the environment variables of `gnbsim` with `oai-amf`,`oai-spgwu`,`oai-smf` and `oai_db3`, all these services should have the same environment variables.

## Start core services
change directory to docker-compose directory i.e `cd docker-compose`

To deploy the core services use the following command:
```
...
- The configuration parameters are preconfigured in `docker-compose-gnbsim.yaml` file and one can modify it for testing purposes.
...
You can check if the UE got allocated an IP address.
...
[gnbsim]2024/07/26 13:01:40.584271 example.go:329: UE address: 12.1.1.2

```
## Ping test
We can ping external Data Network from UE (gnbsim) container.
...
## iperf test
Let's perform an iperf test to check network performance

To know more about iperf test you can click here ---> [iperf](https://www.techtarget.com/searchnetworking/tip/How-to-use-iPerf-to-measure-throughput)
...
#### Sample test results:
- Note: the below results are for your understanding, results will vary with each test
```
Sample Output:
...
```
You can perfom the ping test and iperf test learnt above.
#### Let's verify all `gnb` and `ue` are registered at our 5G core -
Here we analyze the multiple gnbsim connectivity
...
To Remove all the services properly in an ordered fashion:
- Step-1: Remove the gnbsim containers using the command:
```
docker compose -f docker-compose-gnbsim.yaml down
```
- Step-2: Remove the core services using the command: 
```
sudo python3 core-network.py --type stop-basic
```

Everything is now shut down
- Note: It is a good practice to always shut-down the services in this order.
...
Under that directory you'll find the file named `generator.sh`

The script makes your work easier.

#### Workflow of `generator.sh` script:
- It provides sim configuration for you in the `oai_db3.sql` file. 
- It creates multiple containers of gnbsim in the `docker-compose-gnbsim.yaml` file
- Deploys the `oai-core`
- Deploys the multiple instances of `gnbsim`

## Constraints of `generator.sh` bash script
The limitations of the script `generator.sh` makes you follow some constraints mentioned below:
- The `oai_db3.sql` file should have `NO` entries under `AuthenticationSubscription` and `SessionManagementSubscriptionData`. 

For reference there is a `oai_db3.sql.default` file under `./docker-compose/database` directory, you can use that file by removing the added extension `.default`

- `The docker-compose-gnbsim.yaml` file should be Empty i.e the `contents should be erased` (`File should not be deleted`)


## Using `generator.sh`
Let's get a hands on using the `generator.sh` script
### Step-1
...
Note: 
- The script is designed considering there are no gnbsim containers present in the `docker-compose-gnbsim.yaml` file


- It is advised that you create a backup of `docker-compose-basic-nrf.yaml`file, by making a copy, and renaming them with an added extension `.bak`
```
sample:
...
### Step-3
Note:
- Here we have considered that there are no entries under the `AuthenticationSubscription` and `SessionManagementSubscription` in the `oai_db3.sql` file

- Backup the `oai_db3.sql` file as well using the same `oai_db3.sql.bak` extension and working on a copy of it

- You can use the `oai_db3.sql.default` file it is preconfigured, you need to remove the `.default` extension and use it instead of the present `oai_db3.sql` file
...
The containers will spin up one by one.

Once all the containers are up and running, Check the gnbsim containers if they are in healthy condition or not, Simply execute the command:
```
docker ps -a
```
...
### Handling errors
- Note-1: the oai-core can be unstable, if the core doesn't spin up healthy cancel the running script using `Ctrl + C` or `Ctrl + D`
- Note-2: before re-using Undeploy everything.
